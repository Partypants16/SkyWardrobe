  ...
  try {
    ...
    for (const category of categories) {
      ...
      if (candidates.length > 0) {
        // Sort to prioritize preferred matches
        candidates.sort((a, b) => {
          ...
          return Math.random() - 0.5; // Shuffle matching items
        });
...
async function loadSavedWeatherData() {
...
async function refreshWeather() {
  ...
  try {
    ...
    await renderDashboard();
    elements.status.classList.remove("error");
    elements.status.textContent = "Dashboard refreshed with the latest local weather data.";
  } catch (error) {
