...
// ── Config / API helpers ─────────────────────────────────────────────────────
// This app has no backend of its own — it is a fully standalone Capacitor
// WebView bundle, so by default it calls OpenWeatherMap directly from the
// device using a key shipped in config.js (see build-www.js for the tradeoff
// that implies). If API_BASE_URL is configured, requests go through a
// companion SkyWardrobe-Web server's proxy endpoints instead and the local
// key is unused. RainViewer (radar) needs no key either way.
...
function buildOWMUrl(endpoint, extraParams) {
  ...
  if (!config.OPENWEATHER_KEY) {
    throw new Error("Missing OpenWeather API key. Set OPENWEATHER_KEY in .env and run npm run build.");
  }
...
function usingCompanionServer() {
  return Boolean(getAppConfig().API_BASE_URL);
}
...
async function fetchWeatherData(signal) {
  ...
  if (usingCompanionServer()) {
    return fetchJson(buildServerUrl("/weather", params), signal);
  }
  return fetchJson(buildOWMUrl(OWM_WEATHER_URL, params), signal);
}
...
async function fetchForecastData(signal) {
  ...
  const raw = await fetchJson(buildOWMUrl(OWM_FORECAST_URL, { ...params, cnt: 40 }), signal);
  return normalizeForecastResponse(raw);
}

// Direct OWM /forecast responses are nested (list[].main.temp, etc.) — this
// flattens them to the same shape RainService expects, which is what the
// server proxy already returns pre-normalized.
...
async function fetchGeocode(query, signal) {
  ...
  // Same response shape either way — OWM's geocoding endpoint returns a flat
  // array of {name, local_names, lat, lon, country, state}, and the server
  // proxy just forwards that unchanged.
  return fetchJson(buildOWMUrl(OWM_GEO_URL, { q: query, limit: 5 }), signal);
}
...
function normalizeWeatherData(data) {
  ...
  return {
    ...
    // `main` is ambiguous between shapes: the server/normalized shape uses it
    // as the condition string (e.g. "Clouds"), but genuinely raw OWM JSON
    // uses top-level `main` for the temperature/pressure block object. Only
    // treat it as the condition when it's actually a string.
    main:        textValue(typeof data.main === "string" ? data.main : null, cond.main),
...
// ── Local (offline-capable) wardrobe engine ───────────────────────────────────
// Unlike the web app (which asks a server-side SQLite database), this app
// picks an outfit from a bundled JSON file entirely on-device — kept exactly
// as before; only how weather DATA arrives has changed.
async function generateOutfitClient(weather) {
  try {
    const response = await fetch("clothing_items.json");
    if (!response.ok) {
      throw new Error("Failed to load local clothing database.");
    }
    ...
    const windy = (weather.wind_speed ?? 0) > 8;
    const humid = (weather.humidity ?? 0) > 70;

    const categories = ["top", "bottom", "outerwear", "footwear", "accessory"];
    const outfit = {};

    for (const category of categories) {
      const candidates = clothingItems.filter((item) => {
        if (item.category !== category) return false;
        if (item.min_temp !== null && item.min_temp !== undefined && temp < item.min_temp) return false;
        if (item.max_temp !== null && item.max_temp !== undefined && temp > item.max_temp) return false;
        if (raining && item.rain_suitable === 0) return false;
        if (windy && item.wind_suitable === 0) return false;
        if (humid && item.humidity_suitable === 0) return false;
        return true;
      });

      if (candidates.length > 0) {
        candidates.sort((a, b) => {
          const scoreA = (raining ? (a.rain_preferred || 0) : 0) + (windy ? (a.wind_preferred || 0) : 0);
          const scoreB = (raining ? (b.rain_preferred || 0) : 0) + (windy ? (b.wind_preferred || 0) : 0);
        ...
        outfit[category] = {
          ...
          description: candidates[0].description || "No description provided."
        };
      } else {
        outfit[category] = {
          name: `No suitable ${category}`,
          description: "No matching items in local clothing list."
        };
      }
    }
    return outfit;
  } catch (err) {
    console.error("Local outfit recommendation failed:", err.message);
    return {};
  }
}
...
async function renderDashboard() {
  ...
  // Outfit items (local, on-device recommendation engine)
  const outfit = await generateOutfitClient(weather);
  const renderItem = (cat, nameEl, descEl) => {
    const item = outfit[cat] || {};
    nameEl.textContent = item.name || `No suitable ${cat}`;
    descEl.textContent = item.description || "No matching items in local clothing list.";
  };
...
function requestGPSLocation() {
  if (!navigator.geolocation) {
    elements.status.textContent = "GPS is not supported by this device.";
    return;
  }
...
async function startup() {
  ...
  const config = getAppConfig();
  if (!config.API_BASE_URL && !config.OPENWEATHER_KEY) {
    elements.status.classList.add("error");
    elements.status.textContent =
      "No OPENWEATHER_KEY configured — set it in .env and run npm run build (see README.md).";
    return;
  }
